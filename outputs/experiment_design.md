...
### 6.4 초기화 축 (Transfer Learning)

기존 A/C/D는 DDragon Xavier 투영으로 초기화하여 데이터 부족으로 임베딩이 초기화를 탈피하지 못했다.
이를 개선하기 위해 **Model B(137K)에서 학습된 임베딩**을 초기화로 사용하는 실험을 추가하였다.

```
Baseline:  DDragon 17d → Xavier(17→32) → fine-tune on A/C/D
Transfer:  DDragon → Xavier → B(137K) 학습 → fine-tune on A/C/D
                                ↑ 이 단계의 임베딩을 초기화로 사용
```

**구현**: `train.py --init-weights weights/embedding_B_learned.pt`

| 타겟 | DDragon init (FFNN) | B learned init (FFNN) | 변화 |
|------|--------------------|-----------------------|------|
| A (프로) | 53.1% | 52.5% | -0.6%p |
| C (고티어) | 52.7% | 49.2% | **-3.5%p** |
| D (저티어) | 51.8% | 54.6% | **+2.8%p** |

**결론:**
- **D(저티어)만 개선**: B의 솔로랭크 범용 메타 지식이 저티어에 전이됨.
  저티어는 소수 인기 챔피언에 집중되므로, B의 사전 학습이 나머지 챔피언에 대한 유용한 시작점을 제공.
- **C(고티어) 악화**: B의 "전 티어 평균" 임베딩이 고티어 특유의 어쌔신-다이버 메타를 희석.
  소스-타겟 도메인 거리가 너무 큼.
- **A(프로) 중립**: 프로와 솔로랭크는 질적으로 다른 환경이므로 전이 효과 없음.

→ Transfer learning의 효과는 소스-타겟 **도메인 유사성**에 의존하며,
"데이터 부족 → TL이 항상 도움"은 성립하지 않음을 실험적으로 확인.

---

## 7. 한계 및 향후 과제

1. **초기화 비재현성**: `torch.manual_seed` 미설정. 현재 파일 고정 사용으로 실험 결과에 영향 없으나,
   향후 재생성 시 시드 고정 필요.

2. **특성 제한**: attackrange, 스킬 사거리/쿨다운 등 미포함. 단, 모델이 이를 간접 학습한 것은 확인됨.

3. **PCA 해석의 한계**: 상관 기반 추론이므로 인과관계가 아닌 연관성.

4. **카운터 관계 미분석**: Blue-Red 교차 co-occurrence로 카운터 쌍 정량화 가능.

5. **p1~p5 순서의 의미 불확실**: 프로 데이터는 드래프트 순서, 솔로랭크는 포지션 나열일 가능성.
